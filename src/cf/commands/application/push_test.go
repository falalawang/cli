...
func callPush(t mr.TestingT, args []string, deps pushDependencies) (ui *testterm.FakeUI) {
	...
	cmd := NewPush(ui, configRepo, deps.manifestRepo, deps.starter, deps.stopper,
		deps.binder, deps.appRepo, deps.domainRepo, deps.routeRepo,
		deps.stackRepo, deps.serviceRepo, deps.appBitsRepo)
